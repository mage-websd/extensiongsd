...
abstract class AW_Points_Model_Actions_Abstract
{
    ...
    {
        return $this->_getHelper()->__($this->getComment());
    }
    ...
    public function addTransaction($additionalData = array())
    {
        ...
        if (!isset($additionalData['store_id'])) {
            if ($this->getObjectForAction() instanceof Varien_Object
                && $this->getObjectForAction()->getData(
                    'store_id'
                )
            ) {
        ...
        $this->_transaction = Mage::getModel('points/transaction')
            ->changePoints(
    ...
    /**
     * Used for negative amount transactions. Updates previous transactions data,
     * set balance_change_spent value to them in equivalent of negative amount
     *
     * @return AW_Points_Model_Actions_Abstract
     */
    ...
    public function sendMail()
    {
        ...
        if (Mage::helper('points/config')->getIsEnabledNotifications() && $isEnabledUpdateNotification) {
            ...
            $mail->setDesignConfig(array('area' => 'frontend', 'store' => $store->getId()))
                ->sendTransactional(
                    Mage::helper('points/config')->getBalanceUpdateTemplate($store->getId()),
                    Mage::helper('points/config')->getNotificatioinSender($store->getId()),
                    $this->getTransaction()->getCustomerEmail(),
                    null,
                    array(
                         'store'        => $store,
            ...
            if (!$mail->getSentSuccess()) {
                Mage::helper('awcore/logger')->log(
                    $this, Mage::helper('points')->__('Unable to send balance update email.'),
    ...
    /**
     * @return AW_Points_Helper_Config|Mage_Core_Helper_Data|mixed
     */
    protected function _getHelper($additional = null)
    {
        return Mage::helper('points' . ($additional ? '/' . $additional : ''));
    }

    /**
     * @return AW_Points_Helper_Config
     */
    protected function _getConfigHelper()
    {
        return $this->_getHelper('config');
    }

    protected function _translateComment($comment, $locale = 'en_US')
    {
        if (Mage::app()->getLocale()->getLocaleCode() != $locale) {
            $localeObj = new Zend_Locale($locale);
            /** @var Mage_Core_Model_Translate $translator */
            $translator = Mage::getModel('core/translate');
            $translator->setLocale($localeObj)->init('frontend');
    ...
    protected function _translatePattern($comment, $pattern)
    {
        $args = sscanf($comment, $pattern);
        return vsprintf($this->_getHelper()->__($pattern), $args);
    }
}
