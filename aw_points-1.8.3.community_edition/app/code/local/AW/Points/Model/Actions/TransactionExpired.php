...
class AW_Points_Model_Actions_TransactionExpired extends AW_Points_Model_Actions_Abstract
{
    ...
    protected function _applyLimitations($amount)
    {
        ...
        if ($amount > 0 || $this->getSummary()->getPoints() <= 0) {
            ...
            /*
             * If customers points amount is less that amount to reduce (- $amount),
             * we must set customers points amount to 0
             * ($amount = - $this->getSummary()->getPoints())
             * => $this->getSummary()->getPoints() - $this->getSummary()->getPoints() = 0
             */
    ...
    public function getComment()
    {
        if (isset($this->_commentParams['transaction_id'])) {
            return sprintf($this->_comment, $this->_commentParams['transaction_id']);
        }
    ...
    public function getCommentHtml($area = self::ADMIN)
    {
        ...
        $expiredTransactionId = substr(
            $this->getTransaction()->getComment(), strpos($this->getTransaction()->getComment(), '#') + 1
        );
