...
class AW_Points_Model_Mysql4_Transaction extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    public function calculatePointsFor($order, $type)
    {
        $select = $this->_getReadAdapter()->select()
            ...
            ->join(
                array('summary' => $this->getTable('summary')), 'main_table.summary_id = summary.id',
    ...
    public function getRefererPointsFor($order)
    {
        $select = $this->_getReadAdapter()->select()
            ->from(
                array('main_table' => $this->getTable('transaction')), array('*', 'points_earned' =>
    ...
    public function saveSpendOrderInfo($transaction, $order)
    {
        ...
        if (defined("Mage_Customer_Model_Observer::VIV_PROCESSED_FLAG")
            && Mage::registry(Mage_Customer_Model_Observer::VIV_PROCESSED_FLAG)
        ) {
            $orderCustomer->setData('group_id', $order->getCustomer()->getOrigData('group_id'));
        }
        $orderWebsite = $order->getStore()->getWebsite();
        $moneyForPointsBase = Mage::getModel('points/api')->changePointsToMoney(
            $transaction->getBalanceChange(), $orderCustomer, $orderWebsite
        );
