...
class AW_Points_Model_Cron
{
    ...
    public function addBirthdayTransactions()
    {
        ...
        $collection->getSelect()
            ->joinLeft(
                array(
                    'summary' => $collection->getTable('points/summary')
                ),
                'summary.customer_id = e.entity_id',
                array('summary_id' => 'id', '*')
            )
        ...
        foreach ($collection as $info) {
            ...
            try {
                ...
                Mage::getModel('core/email_template')
                    ->setDesignConfig(array('area' => 'frontend', 'store' => $info->getStoreId()))
                    ->sendTransactional(
    ...
    protected function _cleanExpiredTransactions()
    {
        ...
        $expiredTransactions->getResource()->getReadConnection()->raw_query(
            'LOCK TABLE ' . $expiredTransactions->getMainTable() . " as main_table READ"
        );
        ...
        foreach ($ids as $transactionId) {
            ...
            $result = Mage::getModel('points/api')->addTransaction(
                $transaction->getBalanceChangeSpent() - $transaction->getBalanceChange(),
                'transaction_expired',
    ...
    protected function _sendWarningLetter()
    {
        if (Mage::helper('points/config')->getIsEnabledNotifications()) {
            ...
            foreach ($transactionsToWarn as $transaction) {
                ...
                $daysLeft = $expirationDate->sub($currentDate);
                if ($daysLeft instanceof Zend_Date) {
                    $daysLeft = $daysLeft->toValue();
                }

                $daysLeft = floor($daysLeft / (60 * 60 * 24));

                if (!isset($summaryData[$summaryId][$daysLeft])) {
                    $summaryData[$summaryId][$daysLeft] = 0;

                }
                $summaryData[$summaryId][$daysLeft]
                    += $transaction->getData('balance_change') - $transaction->getData('balance_change_spent')
                ;
            }
                ...
                if ($summary->getPointsExpirationNotification()) {
                    ...
                    try {
                        $mail->setDesignConfig(array('area' => 'frontend', 'store' => $store->getStoreId()))
                            ->sendTransactional(
                                Mage::helper('points/config')->getPointsExpireTemplate($store->getStoreId()),
                                Mage::helper('points/config')->getNotificatioinSender($store->getStoreId()),
                                $customer->getEmail(),
                                null,
                                $mailParams
                            );
                        ...
                        $logMessage
                            = Mage::helper('points')->__('Unable to send %s expiration email.', $pointUnitName)
                            . '  '
                            . $exc->getMessage()
                        ;
                        Mage::helper('awcore/logger')->log(
                            $this, $logMessage, AW_Core_Model_Logger::LOG_SEVERITY_WARNING
                        );
                    }
