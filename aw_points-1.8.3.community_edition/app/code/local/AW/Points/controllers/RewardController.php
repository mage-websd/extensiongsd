...
class AW_Points_RewardController extends Mage_Core_Controller_Front_Action
{
    ...
    public function couponActivationAction()
    {
        ...
        if (($now - $lastActivation) < $secondsBetweenAttempts) {
            $customerSession->addError(
                Mage::helper('points')->__('Please, wait %s seconds before submitting', $secondsBetweenAttempts)
            );
            $this->_redirectReferer();
            return $this;
        }
        ...
        if (!$coupon->validateCustomerGroup($customer)) {
            $customerSession->addError(
                Mage::helper('points')->__('You must be included to another group of customers to activate this coupon')
            );
            $this->_redirectReferer();
            return $this;
        }
        ...
        if ($coupon->getData('activation_cnt') >= $coupon->getData('uses_per_coupon')) {
            $customerSession->addError(
                Mage::helper('points')->__('The limit of activations of this coupon is reached')
            );
            $this->_redirectReferer();
            return $this;
        }
