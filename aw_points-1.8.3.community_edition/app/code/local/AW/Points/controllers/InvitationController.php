...
class AW_Points_InvitationController extends Mage_Core_Controller_Front_Action
{
    ...
    public function preDispatch()
    {
        ...
        if (
            Mage::helper('points/config')->isReferalSystemEnabled()
                ...
                try {
                    ...
                    if ($registeredCustomer->getId()) {
                        Mage::getSingleton('customer/session')->addError(
                            Mage::helper('points')->__('Customer with email: %s has already been  registered', $email)
                        );
                        continue;
                    }

                    $preparedInvitation = Mage::getModel('points/invitation')->loadAcceptedByEmail($email);

                    /* invitation exists */
                    if ($preparedInvitation->getId()) {
                        Mage::getSingleton('customer/session')->addError(
                            Mage::helper('points')->__('Invitation for email: %s has already been accepted', $email)
                        );
                        continue;
                    }
                    /* save new invitation */
                    ...
                    if ($sentSuccessfully) {
                        Mage::getSingleton('customer/session')->addSuccess(
                            Mage::helper('points')->__('Invitation for %s has been sent.', $email)
                        );
                    } else {
                        Mage::getSingleton('customer/session')->addNotice(
                            Mage::helper('points')->__('Invitation for %s is not sent. Please try later.', $email)
                        );
                    }
                } catch (Exception $e) {
                    Mage::getSingleton('customer/session')->addError(
                        Mage::helper('points')->__('Error on saving invitation data for email: %s', $email)
                    );
                    Mage::helper('awcore/logger')->log(
                        $this, Mage::helper('points')->__('Error on saving invitation data for email: %s', $email),
                        AW_Core_Model_Logger::LOG_SEVERITY_ERROR, $e->getTraceAsString()
                    );
                }
            }
