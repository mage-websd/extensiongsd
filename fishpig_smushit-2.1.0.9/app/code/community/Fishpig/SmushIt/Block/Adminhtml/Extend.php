...
class Fishpig_SmushIt_Block_Adminhtml_Extend extends Mage_Core_Block_Text
		...
		if ($extensions = $this->getSelectedExtensions()) {
			...
			foreach($extensions as $extension) {
				...
				. $this->_createHtmlElement(
					'p', 
					null, 
					$this->getShortDefinition($extension)
						 . $this->_createHtmlElement('a', $this->getTrackedUrl($extension, $this->getModule(), 'view-more'))
				);
...
#{{ID}} li.item  { margin: 0 0 10px; width: 50%; float: left; }
#{{ID}} li.item .pad {padding: 1%; height: 1%; overflow: hidden; border:1px solid #ddd; margin: 0 5px 0 0; }
#{{ID}} li.item.even .pad { margin: 0 0 0 5px; }
#{{ID}} a.image { float: left; display: block; margin-right: 10px; }
#{{ID}} a.image img { max-height: 60px; display: block; }
#{{ID}} h2 { font-size: 1.4em; font-family: Tahoma, Verdana, Arial; line-height: 1em; margin: 5px 0 2px; }
#{{ID}} h2 a { color: #000; text-decoration: none; }
